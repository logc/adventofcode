...
(def puzzle-input (read-file-lines "day02.in"))
...
(define (parse-line L)
  (map string->number (string-split L #\x)))

(define (order-paper dimensions)
  (match dimensions
    ([l h w] (+ (* 2 l w) (* 2 w h) (* 2 h l) (extra l h w)))))

(define (extra l h w)
  (let ((sides (list (* l w) (* w h) (* h l))))
    (apply min sides)))

(define all-dimensions (map parse-line puzzle-input))

(define (solve-one puzzle-input)
  (define all-paper-orders (map order-paper all-dimensions))
  (apply + all-paper-orders))

(check-eq? (solve-one puzzle-input) 1588178)
...
(define (order-ribbon dimensions)
  (match dimensions
    ([l w h] (+ (bow l w h) (wrap l w h)))))
...
(define (solve-two puzzle-input)
  (define all-ribbon-orders (map order-ribbon all-dimensions))
  (apply + all-ribbon-orders))

(check-eq? (solve-two puzzle-input) 3783758)
